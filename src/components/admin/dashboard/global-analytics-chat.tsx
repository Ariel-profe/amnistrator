...
export const GlobalAnalyticsChart = memo(() => {
  return (
    <div className="border-border bg-card/40 rounded-xl border p-6">
        ...
        <div className="flex h-full items-end justify-between gap-3">
          {chartData.map((item, index) => (
            ...
            >
              <div
                className={clsx(
                  `w-full ${item.color} relative min-h-[20px] cursor-pointer rounded-t-lg transition-opacity hover:opacity-80`,
                  { 'h-20': item.value <= 3000 },
                  { 'h-32': item.value > 3000 && item.value <= 4000 },
                  { 'h-44': item.value > 4000 && item.value <= 5000 },
                  { 'h-52': item.value > 5000 && item.value <= 6000 },
                  { 'h-64': item.value > 6000 }
      ...
      <div className="border-border/50 grid grid-cols-3 gap-4 border-t pt-4">
        <div className="text-center">
          ...
          <div className="text-muted-foreground text-xs">Total de equipos</div>
        </div>
        <div className="text-center">
          <div className="text-2xl font-bold text-blue-500">+18%</div>
          <div className="text-muted-foreground text-xs">Tasa de crecimiento</div>
        </div>
        <div className="text-center">
          <div className="text-2xl font-bold text-purple-500">$4.5K</div>
          <div className="text-muted-foreground text-xs">Abonos</div>
        </div>
      </div>
    </div>
  );
...
GlobalAnalyticsChart.displayName = 'GlobalAnalyticsChart';
